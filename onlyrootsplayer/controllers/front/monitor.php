...
class OnlyrootsplayerMonitorModuleFrontController extends ModuleFrontController
{
    ...
    const ALLOWED_EVENT_TYPES = [
        ...
        'orp:player:init',
        // === v3 iframe debug ===
        'orp:iframe:check',           // periodic check of iframe state
        'orp:iframe:lost',            // iframe was destroyed
        'orp:iframe:duplicate',       // multiple iframes detected
        'orp:iframe:reattach',        // iframe re-attached
        'orp:iframe:strip',           // duplicate stripped from incoming HTML
        'orp:bridge:hook-installed',  // bridge installed a Swup hook
        'orp:bridge:hook-fired',      // bridge hook fired
        'orp:bridge:postmessage',     // postMessage sent to iframe
        'orp:player:state',           // player state broadcast
        'orp:player:audio-event',     // audio element event (play/pause/ended/error)
        'orp:swup:api-detected',      // detected which Swup API version
    ];
